...
%    [~,I]=sort(ws./squeeze(sqrt(sum(sigmas.^2,2))),2,'ascend'); %se ordenan las guassianas de cada pixel segun Ws/std.
    ...
    W = wsT(I+repmat((0:D-1)'*K,[1 K]));
    %es el vector ordenado de ws respecto los valores obtenidos en el sort ws/std. se podría hacer mejor con sub2ind.
    acumW=cumsum(W,2); %suma acumulada tal como marco stGm
    mask = (acumW>THFG)'; %comparacion con umbral assignado. 
    %es una mascara que contiene 0 en aquellas guassianas ordenadas estan por debejo del umbral.
...
%    [~,I2]=sort(I,2); %I2 contiene el indice que era menor en el caso anterior. Permite volver al orden original
    ...
    FG = abs(1-sum(menors.*MASK,2));
    %compración de si el pixel selecciónado como primer plano tambien no cumple el primer plano.
